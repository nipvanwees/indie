    ...
    <Headless.Combobox {...props} multiple={false} virtual={{ options: filteredOptions }} onClose={() => setQuery('')}>
      ...
      >
        {filteredOptions.length === 0 && query !== '' ? (
          <div className="relative flex w-full cursor-default select-none items-center justify-center rounded-lg py-2.5 text-base/6 text-zinc-500 dark:text-white">
            No results found
          </div>
        ) : (
          filteredOptions.map((option) => (
            <Headless.ComboboxOption key={option as unknown as string} value={option} className="w-full">
              {children(option)}
            </Headless.ComboboxOption>
          ))
        )}
        {/* Render the children of the Combobox */}
